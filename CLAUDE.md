...
2.更新claude.md时候只更新必要的内容和说明,不要添加不必要的废话.
...
**核心功能**:
- 📁 文件上传：支持拖拽和点击上传.txt文件
- 👀 文件预览：文件卡片显示内容预览
- ✏️ 富文本编辑：基于Tiptap的现代编辑器
- 💾 本地存储：使用IndexedDB持久化文件数据
- 📱 响应式设计：适配各种屏幕尺寸
- 🎨 现代UI：基于CSS变量的设计系统
...
- **动态路由**: 基于文件ID的参数化路由
- **数据持久化**: 实时保存编辑内容到IndexedDB
- **导航功能**: 返回按钮和面包屑导航
- **加载状态**: 优雅的加载和错误处理
- **固定高度布局**: 编辑器占据固定比例，防止超出屏幕

### Tiptap富文本编辑器组件 (src/components/TiptapEditor.tsx)
- **基于ProseMirror**: 使用现代富文本编辑器架构
- **完整工具栏**: 支持以下格式化功能:
  - 文本样式：粗体、斜体、删除线
  - 标题：H1-H3级别标题
  - 列表：有序列表、无序列表
  ...
  - 历史操作：撤销、重做
- **固定高度设计**:
  - 工具栏固定在顶部 (`flex-shrink-0`)
  - 编辑区域使用剩余空间 (`flex-1`)
  - 内容超出时显示滚动条而非扩展容器
- **自定义滚动条**:
  - 强制显示滚动条 (`overflow-y: scroll`)
  - 自定义滚动条样式匹配设计系统
  - 响应式滚动条宽度和颜色
- **内容处理**:
  - 纯文本到HTML的转换（换行符处理）
