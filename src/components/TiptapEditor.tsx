...
import type { Chapter, WorkItem, WorkItemType } from '@/utils/idb/schema';
import { SettingItemService, CharacterItemService, KnowledgeItemService } from '@/utils/idb/workItems';

const WORK_ITEM_SERVICES = {
  setting: SettingItemService,
  character: CharacterItemService,
  knowledge: KnowledgeItemService,
} as const;

const WORK_ITEM_LABELS: Record<WorkItemType, string> = {
  setting: '设定',
  character: '角色',
  knowledge: '知识库',
};

const WORK_ITEM_EMPTY_TEXT: Record<WorkItemType, string> = {
  setting: '暂无设定内容',
  character: '暂无角色内容',
  knowledge: '暂无知识库内容',
};

const WORK_ITEM_ADD_TEXT: Record<WorkItemType, string> = {
  setting: '+ 添加设定',
  character: '+ 添加角色',
  knowledge: '+ 添加知识库',
};

const renderWorkItemIcon = (type: WorkItemType) => {
  switch (type) {
    case 'setting':
      return (
...
export default function TiptapEditor({
  ...
  const [expandedSections, setExpandedSections] = useState<Record<WorkItemType, boolean>>({
    setting: false,
    character: false,
    knowledge: false,
  });
  ...
  // 中文说明：条目列表状态
  const [workItems, setWorkItems] = useState<Record<WorkItemType, WorkItem[]>>({
    setting: [],
    character: [],
    knowledge: [],
  });

  const toggleSection = useCallback((type: WorkItemType) => {
  ...
  const updateWorkItems = useCallback((type: WorkItemType, updater: (items: WorkItem[]) => WorkItem[]) => {
    setWorkItems(prev => ({
      ...prev,
  ...
  // 中文说明：当前编辑的条目类型和ID
  const [editingType, setEditingType] = useState<'chapter' | WorkItemType>('chapter');
  const [editingItemId, setEditingItemId] = useState<string | null>(null);

  // 中文说明：添加条目弹窗状态
  const [showAddModal, setShowAddModal] = useState(false);
  const [addModalType, setAddModalType] = useState<WorkItemType>('setting');
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      } else if (editingType !== 'chapter' && editingItemId) {
        // 更新设定/角色/知识库条目内容
        const service = WORK_ITEM_SERVICES[editingType];
        service.update(editingItemId, { content: html }).catch(console.error);
        const updatedAt = Date.now();
        setWorkItems(prev => ({
          ...prev,
          [editingType]: prev[editingType].map(item =>
            item.id === editingItemId ? { ...item, content: html, updatedAt } : item
          ),
        }));
      }
  ...
  // 处理条目点击编辑
  const handleItemClick = useCallback((type: WorkItemType, itemId: string) => {
    setEditingType(type);
    setEditingItemId(itemId);

    const item = workItems[type]?.find(i => i.id === itemId);
  ...
  const handleAddItem = useCallback(async () => {
    ...
    try {
      const payload = { title: trimmedTitle, content: '', documentId };
      let newItem: WorkItem;

      if (addModalType === 'setting') {
        newItem = await SettingItemService.create(payload);
      } else if (addModalType === 'character') {
        newItem = await CharacterItemService.create(payload);
      } else {
        newItem = await KnowledgeItemService.create(payload);
      }

      updateWorkItems(addModalType, prev => [...prev, newItem]);
      setExpandedSections(prev => ({ ...prev, [addModalType]: true }));
      setShowAddModal(false);
      setNewItemTitle('');
      setEditingType(addModalType);
      setEditingItemId(newItem.id);
      editor?.commands.setContent(newItem.content ?? '');
    } catch (error) {
      console.error('添加条目失败:', error);
      alert('添加失败，请重试');
    }
  }, [documentId, newItemTitle, addModalType, updateWorkItems, editor]);

  // 删除条目
  const handleDeleteItem = useCallback(async (type: WorkItemType, itemId: string) => {
    if (!confirm('确定要删除该条目吗？')) return;

    try {
      await WORK_ITEM_SERVICES[type].delete(itemId);
      updateWorkItems(type, prev => prev.filter(i => i.id !== itemId));

      // 如果删除的是当前编辑的条目，切换回章节编辑
      if (editingType === type && editingItemId === itemId) {
        setEditingType('chapter');
        setEditingItemId(null);
        if (editor && currentChapter) {
          const fallbackContent = currentChapter.content || '';
          if (fallbackContent !== editor.getHTML()) {
  ...
  }, [editingType, editingItemId, updateWorkItems, editor, currentChapter]);
        ...
        <div className="hidden lg:flex lg:flex-col w-64 border-r border-light bg-light/30 flex-shrink-0">
          ...
          <div className="border-b border-light overflow-y-auto flex-shrink-0" style={{ maxHeight: '40%' }}>
            <div className="p-4 space-y-2">
              {(Object.keys(WORK_ITEM_LABELS) as WorkItemType[]).map(type => {
                const isExpanded = expandedSections[type];
                const items = workItems[type];

                return (
                  <div key={type} className="bg-white rounded-lg overflow-hidden shadow-sm">
                    <button
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-primary">{WORK_ITEM_LABELS[type]}</span>
                      </div>
                      <svg
                    ...
                    {isExpanded && (
                      <div className="px-2 py-2 border-t border-light bg-light/50 space-y-1">
                        {items.length === 0 ? (
                          <p className="text-xs text-secondary px-2 py-1">{WORK_ITEM_EMPTY_TEXT[type]}</p>
                        ) : (
                          items.map(item => (
                        ...
                        <button
                          onClick={() => { setAddModalType(type); setShowAddModal(true); }}
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div className="px-6 py-4 border-b border-light">
              <h2 className="text-lg font-bold text-primary">
                添加{WORK_ITEM_LABELS[addModalType]}
              </h2>
            </div>
            <div className="px-6 py-4">
              <input
                ...
                placeholder={`输入${WORK_ITEM_LABELS[addModalType]}名称`}
